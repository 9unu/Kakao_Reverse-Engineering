...
def txt_process(lines):
    ...
    for user_name in df['user'].unique(): 
        user_index=list(df[df['user']==f"{user_name}"].index)
    ...
    for user_name in df['user'].unique():
        user_df=df[df['user']==f"{user_name}"]
...
def txt_file_multiprocess(txt_path):
    # 텍스트 파일을 읽어와 리스트로 저장
    ...
    if(len(lines)<30000):
        ...
        dfs = pool.starmap(txt_process, chunk_list)
        pool.close()
        pool.join()
